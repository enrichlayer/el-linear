...
- **Reuse confirmed metadata and ownership.** Carry same-task team, project and assignee decisions forward with their evidence. Before changing an issue, refresh its current fields and ownership, reusing evidence that is still current for that mutation. Check repository/worktree claims when starting or changing repository work. Do not take a peer's claim. Resolve routine missing fields from the existing issue or canonical workspace mappings. Ask only when consequential ambiguity remains, authority is missing, or a user-named project still cannot be resolved after discovery. Do not silently substitute another project.
...
| Resolve uncertain command syntax | Version-matched `el-linear <command> --help`; load [CLI syntax and errors](references/cli-syntax.md) for body-file handling, error guidance, or questions the help leaves unresolved. A routine syntax lookup does not require both sources. |
