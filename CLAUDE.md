...
- **This is open-source — collect nothing by default.** el-linear is MIT and published on npm; most installs are **not** Enrich Layer and have no EL infrastructure (no `el-telemetry`, no Vault, no `~/.cache/el-telemetry`). Any feature that writes a file, caches, or records telemetry/analytics must be **opt-in**, never opt-out — a fresh install must do nothing the user didn't ask for, and the format must be documented so a non-EL user can consume it. (Regression that motivated this: the duplicate-detection gate's telemetry first shipped opt-out, writing `~/.cache/el-telemetry/gate-events.jsonl` for everyone; fixed to opt-in via `decideGateLedger` in `utils/gate-telemetry.ts` + `docs/telemetry.md`. EL-internal CLIs in the `tools` monorepo use opt-out-via-`EL_TELEMETRY_DISABLED`; that convention does **not** apply to this package.)
