...
export function setupTemplatesCommands(program: Command): void {
  const templates = program
    .command("templates")
    .description("Template operations (issue, document, and project templates)");
  ...
  templates
    ...
    .option("--type <type>", "filter by type (issue, document, project, recurringIssue)")
  ...
  templates
    ...
    .action(
      handleAsyncCommand(async (templateId: string, _options: OptionValues, command: Command) => {
        const rootOpts = command.parent!.parent!.opts();
        const graphQLService = createGraphQLService(rootOpts);
        const result = await graphQLService.rawRequest(TEMPLATE_BY_ID_QUERY, { id: templateId });
        const template = result.template as unknown as TemplateResult | undefined;
        if (!template) {
