...
export function setupReadShortcut(program: Command): void {
  ...
  program.parse = function (argv?: readonly string[], parseOptions?: { from: "node" | "electron" | "user" }) {
    ...
    const onlyCompatFlags = flags.every((f) => f === "--json" || f.startsWith("--api-token"));
    ...
    if (
      ...
      const knownCommands = new Set(
        program.commands.map((c) => c.name()).concat(program.commands.flatMap((c) => c.aliases())),
      );
...
async function readIssues(issueIds: string[], _options: Record<string, unknown>, command: Command) {
