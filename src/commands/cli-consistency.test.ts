...
describe("CLI consistency", () => {
  it("all list subcommands have a --limit option", () => {
    ...
    expect(missing, `List commands missing --limit: ${missing.join(", ")}`).toEqual([]);
  });
  ...
  it("all list subcommands have consistent --limit short flag", () => {
    ...
    for (const { path, command } of listCommands) {
      ...
      if (limitOpt && limitOpt.short !== "-l") {
        inconsistent.push(`${path}: short flag is "${limitOpt.short}" (expected "-l")`);
      }
    }

    expect(inconsistent, `Inconsistent --limit short flags: ${inconsistent.join(", ")}`).toEqual(
      [],
    );
  });
});
