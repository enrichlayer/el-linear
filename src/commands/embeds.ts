...
export function setupEmbedsCommands(program: Command): void {
  ...
  embeds
    ...
    .action(
      handleAsyncCommand(async (url: string, options: OptionValues, command: Command) => {
  ...
  embeds
    ...
    .action(
      handleAsyncCommand(async (filePath: string, _options: OptionValues, command: Command) => {
        const apiToken = getApiToken(command.parent!.parent!.opts());
        const fileService = new FileService(apiToken);
