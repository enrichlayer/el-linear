...
describe("graphql command", () => {
  it("executes inline query", async () => {
    ...
    expect(mockOutputSuccess).toHaveBeenCalledWith({ viewer: { id: "user-1" } });
  });
  ...
  it("reads query from file", async () => {
    ...
    expect(fs.default.readFileSync).toHaveBeenCalledWith("query.graphql", "utf8");
    expect(mockRawRequest).toHaveBeenCalledWith("{ teams { nodes { id } } }", undefined);
  });
  ...
  it("throws on invalid JSON variables", async () => {
    ...
    await expect(
      runCommand(program, ["graphql", "{ viewer { id } }", "--variables", "not-json"]),
