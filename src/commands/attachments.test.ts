...
describe("attachments commands", () => {
  ...
  describe("attachments list", () => {
    it("resolves issue ID and lists attachments", async () => {
      ...
      const attachments = [
        ...
        { id: "att-2", title: "image.png", url: "https://example.com/image.png" },
      ];
    ...
    it("respects --limit option", async () => {
      ...
      const program = createTestProgram();
      setupAttachmentsCommands(program);
      await runCommand(program, ["attachments", "list", "ISSUE-1", "--limit", "2"]);
  ...
  describe("attachments create", () => {
    it("uploads file and creates attachment on issue", async () => {
      ...
      const program = createTestProgram();
      setupAttachmentsCommands(program);
      await runCommand(program, ["attachments", "create", "ISSUE-1", "--file", "test.pdf"]);
    ...
    it("errors when file upload fails", async () => {
      ...
      await runCommand(program, ["attachments", "create", "ISSUE-1", "--file", "missing.pdf"]);
