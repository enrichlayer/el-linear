...
vi.mock("../utils/error-messages.js", () => ({
  ...
  invalidParameterError: (param: string, msg: string) => new Error(`${param}: ${msg}`),
...
describe("cycles", () => {
  ...
  describe("list", () => {
    ...
    it("passes --team through resolveTeam", async () => {
      ...
      expect(mockGetCycles).toHaveBeenCalledWith("team-uuid-123", undefined, 50);
    });
    ...
    it("--around-active without --team throws", async () => {
      suppressExit();
      await expect(runCommand(program, ["cycles", "list", "--around-active", "2"])).rejects.toThrow(
        "--around-active requires --team",
      );
    });
    ...
    it("--around-active filters cycles around active", async () => {
      ...
      await runCommand(program, ["cycles", "list", "--team", "ENG", "--around-active", "1"]);
  ...
  describe("read", () => {
    it("resolves cycle ID and calls getCycleById", async () => {
      ...
      mockGetCycleById.mockResolvedValue({ id: "cycle-uuid-abc", name: "Cycle 5" });
      ...
      expect(mockOutputSuccess).toHaveBeenCalledWith({ id: "cycle-uuid-abc", name: "Cycle 5" });
    });
  });
});
