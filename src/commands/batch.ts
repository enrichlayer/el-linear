...
async function resolveTargetIssues(
  ...
  if (options.filter) {
    ...
    const searchArgs: Record<string, unknown> = {
      ...
      assigneeId: filters.assignee ? await resolveAssignee(filters.assignee, rootOpts) : undefined,
  ...
  throw new Error("Specify --issues (comma-separated IDs) or --filter to select issues.");
}
...
async function executeBatch(
  ...
  for (const issue of issues) {
    try {
      ...
      results.push({ identifier: issue.identifier, success: false, error: message });
    }
  }
...
async function handleBatchAssign(options: OptionValues, command: Command): Promise<void> {
...
async function handleBatchLabel(options: OptionValues, command: Command): Promise<void> {
  const rootOpts = command.parent!.parent!.opts();
...
async function handleBatchMove(options: OptionValues, command: Command): Promise<void> {
  const rootOpts = command.parent!.parent!.opts();
  ...
  const { results } = await executeBatch(issues, (issue) =>
    issuesService.updateIssue({ id: issue.identifier, projectId: options.project }, "adding"),
  );
...
async function handleBatchStatus(options: OptionValues, command: Command): Promise<void> {
  const rootOpts = command.parent!.parent!.opts();
  ...
  const { results } = await executeBatch(issues, (issue) =>
    issuesService.updateIssue({ id: issue.identifier, statusId: options.status }, "adding"),
  );
...
export function setupBatchCommands(program: Command): void {
  ...
  batch
    ...
    .option("--issues <issues>", "comma-separated issue IDs (e.g. DEV-100,DEV-101)")
    .option("--filter <filter>", 'query filter (e.g. "status:Backlog team:DEV label:Bug")')
