...
describe("labels", () => {
  ...
  describe("list", () => {
    ...
    it("calls getLabels with team filter resolved", async () => {
      ...
      mockGetLabels.mockResolvedValue({ labels: [{ id: "lbl-1", name: "Bug" }] });
  ...
  describe("create", () => {
    it("creates label with name and team", async () => {
      ...
      await runCommand(program, ["labels", "create", "Feature", "--team", "ENG"]);
      expect(mockResolveTeam).toHaveBeenCalledWith("ENG");
      ...
      expect(mockOutputSuccess).toHaveBeenCalledWith({ id: "lbl-new", name: "Feature" });
    });
    ...
    it("includes parent label when --parent specified", async () => {
      ...
      expect(mockOutputSuccess).toHaveBeenCalledWith({ id: "lbl-child", name: "SubFeature" });
    });
  });
  ...
  describe("retire", () => {
    it("calls retire mutation", async () => {
      mockRawRequest.mockResolvedValue({
        issueLabelRetire: { success: true, issueLabel: { id: "lbl-123", name: "Retired" } },
      });
      ...
      expect(mockOutputSuccess).toHaveBeenCalledWith({ id: "lbl-123", name: "Retired" });
    });
  });
  ...
  describe("restore", () => {
    it("calls restore mutation", async () => {
      mockRawRequest.mockResolvedValue({
        issueLabelRestore: { success: true, issueLabel: { id: "lbl-456", name: "Restored" } },
      });
      ...
      expect(mockOutputSuccess).toHaveBeenCalledWith({ id: "lbl-456", name: "Restored" });
    });
  });
});
