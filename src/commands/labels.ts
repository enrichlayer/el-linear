...
async function handleCreateLabel(
  ...
  if (options.parent) {
    const parentResult = await graphQLService.rawRequest(FIND_PARENT_LABEL_QUERY, {
      name: options.parent,
      teamId,
    });
    const issueLabels = parentResult.issueLabels as GraphQLResponseData | undefined;
  ...
  const result = await graphQLService.rawRequest(CREATE_LABEL_MUTATION, { input });
  const createResult = result.issueLabelCreate as GraphQLResponseData | undefined;
...
async function handleRetireLabel(
  ...
  const rootOpts = command.parent!.parent!.opts();
  const graphQLService = createGraphQLService(rootOpts);
  const result = await graphQLService.rawRequest(RETIRE_LABEL_MUTATION, { id: labelId });
  const retireResult = result.issueLabelRetire as GraphQLResponseData | undefined;
...
async function handleRestoreLabel(
  ...
  const rootOpts = command.parent!.parent!.opts();
  const graphQLService = createGraphQLService(rootOpts);
  const result = await graphQLService.rawRequest(RESTORE_LABEL_MUTATION, { id: labelId });
  const restoreResult = result.issueLabelRestore as GraphQLResponseData | undefined;
...
export function setupLabelsCommands(program: Command): void {
  const labels = program.command("labels").alias("label").description("Label operations");
  ...
  labels
    ...
    .action(
      handleAsyncCommand(async (options: OptionValues, command: Command) => {
        ...
        outputSuccess({ data: result.labels, meta: { count: result.labels.length } });
      }),
    );
