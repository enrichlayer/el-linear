...
export function setupCyclesCommands(program: Command): void {
  const cycles = program.command("cycles").alias("cycle").description("Cycle operations");
  ...
  cycles
    ...
    .action(
      handleAsyncCommand(async (cycleIdOrName: string, options: OptionValues, command: Command) => {
        const rootOpts = command.parent!.parent!.opts();
        const teamFilter = options.team ? resolveTeam(options.team) : undefined;
        const linearService = createLinearService(rootOpts);
        const cycleId = await linearService.resolveCycleId(cycleIdOrName, teamFilter);
