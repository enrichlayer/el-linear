...
const ALL_QUERIES: [string, string][] = [
	...
	[
		"COMPLETE_ISSUE_FRAGMENT",
		`query Dummy { issue { ${COMPLETE_ISSUE_FRAGMENT} } }`,
	],
	[
		"COMPLETE_ISSUE_WITH_COMMENTS_FRAGMENT",
	...
	["TEAM_SCOPED_FILTERED_ISSUES_QUERY", TEAM_SCOPED_FILTERED_ISSUES_QUERY],
	...
	["BATCH_GET_ISSUES_QUERY", BATCH_GET_ISSUES_QUERY],
	...
	["ARCHIVE_ISSUE_MUTATION", ARCHIVE_ISSUE_MUTATION],
	...
	["GET_ISSUE_START_CONTEXT_QUERY", GET_ISSUE_START_CONTEXT_QUERY],
	["GET_ISSUE_CLAIM_CONTEXT_QUERY", GET_ISSUE_CLAIM_CONTEXT_QUERY],
	["TEAM_STARTED_STATUSES_QUERY", TEAM_STARTED_STATUSES_QUERY],
	["ISSUE_RELATION_CREATE_MUTATION", ISSUE_RELATION_CREATE_MUTATION],
	[
		"buildResolveLabelsByNameQuery",
		buildResolveLabelsByNameQuery(["bug"]).query,
	],
	...
	["PROJECT_READ_QUERY", PROJECT_READ_QUERY],
	...
	["CREATE_PROJECT_MUTATION", CREATE_PROJECT_MUTATION],
 ...
 */
function findConnectionFieldsWithoutNodes(
	queryName: string,
	queryString: string,
): string[] {
	...
	visit(doc, {
		Field(node) {
			...
			if (!node.selectionSet) {
				violations.push(
					`"${queryName}": connection field "${fieldName}" has no selection set`,
				);
				return;
			}
	...
	it("all query module exports are covered", async () => {
		...
		const allExportNames = queryModules.flatMap((mod) =>
			Object.keys(mod).filter(
				(k) => typeof (mod as Record<string, unknown>)[k] === "string",
			),
		);
...
describe("project-milestones.ts — scoped vs global and CRUD", () => {
	it("scoped query requires projectId", () => {
		expect(extractVariables(FIND_PROJECT_MILESTONE_BY_NAME_SCOPED)).toContain(
			"projectId",
		);
		expect(extractVariables(FIND_PROJECT_MILESTONE_BY_NAME_SCOPED)).toContain(
			"name",
	...
	it("global query requires only name", () => {
		expect(extractVariables(FIND_PROJECT_MILESTONE_BY_NAME_GLOBAL)).toEqual([
			"name",
		]);
	});
	...
	it("GET_PROJECT_MILESTONE_BY_ID_QUERY includes issues", () => {
		...
		expect(extractVariables(GET_PROJECT_MILESTONE_BY_ID_QUERY)).toContain(
			"issuesFirst",
		);
	});
	...
	it("UPDATE_PROJECT_MILESTONE_MUTATION accepts sortOrder", () => {
		expect(extractVariables(UPDATE_PROJECT_MILESTONE_MUTATION)).toContain(
			"sortOrder",
		);
	});
});
