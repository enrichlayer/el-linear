...
const { resolveTeam, resolveMember, resolveLabels } = await import(
	"./resolver.js"
);
...
describe("resolveLabels", () => {
	it("resolves workspace labels by name", () => {
		expect(resolveLabels(["claude"])).toEqual([
			"c1a0de00-0000-4000-8000-000000000001",
		]);
	});

	it("resolves team-scoped labels", () => {
		expect(resolveLabels(["tech-debt"], "DEV")).toEqual([
			"7ec4deb7-0000-4000-8000-000000000003",
		]);
	});
	...
	it("returns label name when config has truncated UUID", () => {
		expect(resolveLabels(["truncated-label"], "DEV")).toEqual([
			"truncated-label",
		]);
	});
	...
	it("only returns config UUIDs that pass full UUID validation", () => {
		...
		expect(resolveLabels(["design"], "FE")).toEqual([
			"de519000-0000-4000-8000-000000000004",
		]);
