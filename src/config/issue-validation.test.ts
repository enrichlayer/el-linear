...
const { validateIssueCreation, normalizeLabel, enforceValidation } =
	await import("./issue-validation.js");
	...
	describe("when validation is enabled", () => {
		...
		it("errors when no labels provided", () => {
			const result = validateIssueCreation({
				...
				labels: null,
				description:
					"Some description that is long enough to pass the minimum length check.",
				title: "Fix something",
			});
		...
		it("warns when no why section detected", () => {
			...
			expect(result.errors).toHaveLength(0);
			expect(result.warnings.some((w) => w.includes("Why we need this"))).toBe(
				true,
			);
		});
		...
		it("detects why section variants", () => {
			...
			for (const desc of variants) {
				...
				const hasWhyWarning = result.warnings.some((w) =>
					w.includes("Why we need this"),
				);
		...
		it("warns on long titles", () => {
			...
			expect(result.warnings.some((w) => w.includes("101 characters"))).toBe(
				true,
			);
		});
		...
		it("warns on titles starting with articles", () => {
			for (const article of ["A ", "An ", "The "]) {
				...
				expect(result.warnings.some((w) => w.includes("action verb"))).toBe(
					true,
				);
			}
		});
		...
		it("warns when a label is aliased", () => {
			...
			expect(
				result.warnings.some((w) =>
					w.includes('"feature-request" normalized to "feature"'),
				),
			).toBe(true);
		});
...
describe("title-verb / type-label alignment", () => {
	const base = {
		description: "A valid description with enough context for testing.",
		...required,
	};
	...
	it("no warning when title verb matches type", () => {
		...
		for (const [title, type] of cases) {
			...
			expect(
				verbWarnings,
				`Expected no verb warning for "${title}" with type "${type}"`,
			).toHaveLength(0);
		}
	});
	...
	it("warns when title verb mismatches type", () => {
		...
		expect(
			r.warnings.some((w) =>
				w.includes('starts with "Build" but type is "bug"'),
			),
		).toBe(true);
	});
	...
	it("hints at the correct type for the verb", () => {
		...
		expect(
			r.warnings.some((w) =>
				w.includes('"Fix" is typically associated with "bug"'),
			),
		).toBe(true);
	});
	...
	it("no warning when title starts with non-verb word", () => {
		const r = validateIssueCreation({
			...base,
			title: "Dashboard auth is broken",
			labels: ["bug", "frontend"],
		});
		const verbWarnings = r.warnings.filter((w) => w.includes("but type is"));
	...
	it("warns on multi-word verb mismatch", () => {
		...
		expect(
			r.warnings.some((w) =>
				w.includes('"Set up" is typically associated with "chore"'),
			),
		).toBe(true);
	});
