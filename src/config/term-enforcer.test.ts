...
describe("enforceTerms", () => {
  ...
  it("does not throw when text contains a URL with a rejected token", () => {
    ...
    expect(() => enforceTerms(["Visit enrichlayer.com for details"])).not.toThrow();
  });

  it("throws in strict mode on misspelling", () => {
    expect(() => enforceTerms(["Fix EnrichLayer login bug"], { strict: true })).toThrow(
  ...
  it("checks every text passed in", () => {
    expect(() =>
      enforceTerms(["Good title", "but EnrichLayer in description"], { strict: true }),
    ).toThrow("Term enforcement failed");
  });

  it("ignores null/undefined entries", () => {
    expect(() => enforceTerms([null, undefined, "Fix Enrich Layer bug"])).not.toThrow();
  });

  it("allows the rejected token in URLs like enrichlayer.co", () => {
    expect(() => enforceTerms(["Go to enrichlayer.co"], { strict: true })).not.toThrow();
  });

  it("enforces multiple rules in one pass", () => {
    expect(() =>
      enforceTerms(["Fix EnrichLayer integration with linear.app"], { strict: true }),
...
describe("enforceTerms with no rules", () => {
  it("is a no-op when terms is empty", async () => {
    ...
    expect(() => enforce(["literally anything goes here"], { strict: true })).not.toThrow();
