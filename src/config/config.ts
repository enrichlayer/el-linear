...
export function loadConfig(): LinctlConfig {
  ...
  if (fs.existsSync(CONFIG_PATH)) {
    try {
      const userConfig = JSON.parse(fs.readFileSync(CONFIG_PATH, "utf8")) as Record<string, unknown>;
      ...
      if (userConfig.brand && typeof userConfig.brand === "object" && !Array.isArray(userConfig.brand)) {
        ...
        if (typeof legacy.name === "string" && Array.isArray(legacy.reject)) {
          ...
          const existing = Array.isArray(userConfig.terms) ? userConfig.terms : [];
          userConfig.terms = [
            ...existing,
