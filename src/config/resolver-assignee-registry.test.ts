...
import { resolveAssignee, resolveMemberWithRegistry } from "./resolver.js";
...
// The shared async helper behind --assignee AND --delegate (DEV-4872). Proving
// it here covers the --delegate call sites, which call it directly.
describe("resolveMemberWithRegistry — shared --assignee/--delegate path", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	it("returns the registry UUID when configured and the registry resolves", async () => {
		mockIsConfigured.mockReturnValue(true);
		mockResolveViaRegistry.mockResolvedValue("registry-uuid-dmitrii");
		await expect(resolveMemberWithRegistry("dima")).resolves.toBe(
			"registry-uuid-dmitrii",
		);
		expect(mockResolveViaRegistry).toHaveBeenCalledWith("dima");
	});
	...
	it("never consults the registry when unconfigured (config-only)", async () => {
		mockIsConfigured.mockReturnValue(false);
		await expect(resolveMemberWithRegistry("dima")).resolves.toBe(
			"config-uuid-dmitrii",
		);
