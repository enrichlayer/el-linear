...
describe("wrapIssueReferencesAsLinks", () => {
	it("wraps a single bare identifier", () => {
		...
		).toBe(`see [DEV-100](${url("DEV-100")}) for details`);
	});
	...
	it("returns text unchanged when valid set is empty", () => {
		expect(wrapIssueReferencesAsLinks("DEV-100 EMW-258", valid(), W)).toBe(
			"DEV-100 EMW-258",
		);
	});
	...
	it("does not wrap inside fenced code blocks", () => {
		const text = [
			"before DEV-100",
			"```",
			"log: DEV-200 inside fence",
			"```",
			"after DEV-300",
		].join("\n");
		const result = wrapIssueReferencesAsLinks(
			text,
			valid("DEV-100", "DEV-200", "DEV-300"),
			W,
		);
	...
	it("does not wrap inside the URL portion of an existing markdown link", () => {
		...
		expect(result.match(/\[DEV-100\]/g)?.length ?? 0).toBe(0);
	});
	...
	it("does not wrap inside angle-bracket autolinks", () => {
		...
		expect(
			wrapIssueReferencesAsLinks(text, valid("DEV-100", "DEV-200"), W),
		).toBe(`<https://example.com/DEV-100> and [DEV-200](${url("DEV-200")})`);
	});

	it("idempotent: running twice produces the same result", () => {
		const text = "see DEV-100 and DEV-200";
		const once = wrapIssueReferencesAsLinks(
			text,
			valid("DEV-100", "DEV-200"),
			W,
		);
		const twice = wrapIssueReferencesAsLinks(
			once,
			valid("DEV-100", "DEV-200"),
			W,
		);
	...
	it("handles repeated occurrences of the same id", () => {
		expect(
			wrapIssueReferencesAsLinks(
				"DEV-100 then DEV-100 again",
				valid("DEV-100"),
				W,
			),
		).toBe(
			`[DEV-100](${url("DEV-100")}) then [DEV-100](${url("DEV-100")}) again`,
		);
	});
});
