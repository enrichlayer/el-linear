...
import { createGraphQLService, type GraphQLService } from "./graphql-service.js";
...
class GraphQLDocumentsService {
  ...
  async createDocument(input: Record<string, unknown>): Promise<LinearDocument> {
  ...
  async updateDocument(id: string, input: Record<string, unknown>): Promise<LinearDocument> {
  ...
  async listDocuments(options?: { projectId?: string; first?: number }): Promise<LinearDocument[]> {
    const filter = options?.projectId ? { project: { id: { eq: options.projectId } } } : undefined;
    const result = await this.graphqlService.rawRequest(LIST_DOCUMENTS_QUERY, {
      first: options?.first ?? 50,
      filter,
    });
    return ((result.documents as GraphQLResponseData).nodes as GraphQLResponseData[]).map(
      transformDocument,
    );
  }
  ...
  async listDocumentsBySlugIds(slugIds: string[], limit?: number): Promise<LinearDocument[]> {
    if (slugIds.length === 0) {
      return [];
    }
    const filter = {
