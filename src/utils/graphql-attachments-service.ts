...
class GraphQLAttachmentsService {
  ...
  async createAttachment(input: Record<string, unknown>): Promise<LinearAttachment> {
    ...
    return transformAttachment(attachmentCreate.attachment as GraphQLResponseData);
  }
  ...
  async listAttachments(issueId: string): Promise<LinearAttachment[]> {
    const result = await this.graphqlService.rawRequest(LIST_ATTACHMENTS_QUERY, { issueId });
    if (!result.issue) {
      throw new Error(`Issue not found: ${issueId}`);
    }
    const issue = result.issue as GraphQLResponseData;
    const attachments = issue.attachments as GraphQLResponseData;
    return (attachments.nodes as GraphQLResponseData[]).map(transformAttachment);
  }
}
