...
export class FileService {
  ...
  async uploadFile(filePath: string): Promise<FileUploadResult> {
    ...
    try {
      const uploadMeta = await this.requestUploadUrl(contentType, filename, validation.fileSize);
      if (!uploadMeta.ok) {
        return uploadMeta.error;
      }
      return await this.putFileToUrl(filePath, contentType, filename, uploadMeta);
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
  ...
  private async validateFileForUpload(
    filePath: string,
  ): Promise<{ ok: true; fileSize: number } | { ok: false; error: FileUploadResult }> {
    try {
      await access(filePath);
    } catch {
      return { ok: false, error: { success: false, error: `File not found: ${filePath}` } };
    }
    try {
  ...
  > {
    ...
    const response = await fetch("https://api.linear.app/graphql", {
      ...
      body: JSON.stringify({ query, variables: { contentType, filename, size: fileSize } }),
    });
    if (!response.ok) {
      return {
        ok: false,
  ...
  private async putFileToUrl(
    ...
    contentType: string,
    filename: string,
    meta: { uploadUrl: string; assetUrl: string; headers: Array<{ key: string; value: string }> },
