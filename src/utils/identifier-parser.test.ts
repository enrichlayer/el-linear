...
describe("parseIssueIdentifier", () => {
  ...
  it("throws on invalid format without dash", () => {
    expect(() => parseIssueIdentifier("DEV123")).toThrow("Invalid issue identifier format");
  });

  it("throws on too many dashes", () => {
    expect(() => parseIssueIdentifier("DEV-12-3")).toThrow("Invalid issue identifier format");
  });

  it("throws on non-numeric issue number", () => {
    expect(() => parseIssueIdentifier("DEV-abc")).toThrow("Invalid issue number");
  });
});
...
describe("tryParseIssueIdentifier", () => {
  ...
  it("returns null for UUID", () => {
    expect(tryParseIssueIdentifier("4b6bb89a-9348-4ab7-9e01-581040273998")).toBeNull();
  });
});
