...
export class GraphQLIssuesService {
	...
	async updateIssue(
		...
		if (args.assigneeId && !isUuid(args.assigneeId as string)) {
			args.assigneeId = await this.linearService.resolveUserId(
				args.assigneeId as string,
			);
		}
		...
		for (const labelIdOrName of labelIds) {
			if (isUuid(labelIdOrName)) {
				...
				if (label.isGroup) {
					...
					const hint = teamKey
						? ` Run: linctl labels list --team ${teamKey}`
						: "";
					throw new Error(
	...
	private buildCreateInput(
		...
		if (
			resolved.labelIds &&
			Array.isArray(resolved.labelIds) &&
			resolved.labelIds.length > 0
		) {
		...
		if (
			args.subscriberIds &&
			Array.isArray(args.subscriberIds) &&
			args.subscriberIds.length > 0
		) {
	...
	private async resolveUpdateContext(args: Record<string, unknown>): Promise<{
		...
		if (args.labelIds && Array.isArray(args.labelIds)) {
			const nonUuidLabels = (args.labelIds as string[]).filter(
				(id: string) => !isUuid(id),
			);
		...
		if (
			args.milestoneId &&
			typeof args.milestoneId === "string" &&
