...
import {
	...
	isLinearUploadUrl,
} from "./embed-parser.js";

describe("isLinearUploadUrl", () => {
	it("returns true for uploads.linear.app URLs", () => {
		expect(isLinearUploadUrl("https://uploads.linear.app/abc/def.png")).toBe(
			true,
		);
	});
...
describe("extractFilenameFromUrl", () => {
	it("extracts filename from URL path", () => {
		expect(
			extractFilenameFromUrl("https://uploads.linear.app/abc/screenshot.png"),
		).toBe("screenshot.png");
	});
