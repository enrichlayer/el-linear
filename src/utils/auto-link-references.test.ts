...
function makeRelationsResponse(args: {
  outgoing?: Array<{ identifier: string; uuid: string; type: string; title?: string }>;
  incoming?: Array<{ identifier: string; uuid: string; type: string; title?: string }>;
}): RelationsResponse {
  return {
    issue: {
      relations: {
        nodes: (args.outgoing ?? []).map((r, i) => ({
          id: `rel-out-${i}`,
          type: r.type,
          relatedIssue: { id: r.uuid, identifier: r.identifier, title: r.title ?? r.identifier },
        })),
      },
      inverseRelations: {
        nodes: (args.incoming ?? []).map((r, i) => ({
          id: `rel-in-${i}`,
          type: r.type,
          issue: { id: r.uuid, identifier: r.identifier, title: r.title ?? r.identifier },
        })),
      },
    },
  };
...
function makeRelationCreateResponse(args: {
  ...
  const sourcePlaceholder = { id: "uuid-source", identifier: "EMW-258", title: "source" };
  return {
    issueRelationCreate: {
      success: true,
      issueRelation: {
...
describe("autoLinkReferences", () => {
  ...
  it("returns empty result and makes no API calls when description has no references", async () => {
    const { rawRequest, resolveIssueId, graphQLService, linearService } = makeServices();
  ...
  it("creates a related relation for a new bare reference", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationCreateResponse({
        type: "related",
        reverse: false,
        peer: { id: "uuid-target", identifier: "DEV-3592", title: "Original framing" },
      }),
    );
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-3592", title: "Original framing", type: "related", reverse: false },
    ]);
    expect(result.skipped).toEqual([]);
    expect(result.failed).toEqual([]);
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-source", relatedIssueId: "uuid-target", type: "related" },
    });
  });
  ...
  it("creates a 'blocks' relation when prose says 'blocked by' (reversed)", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationCreateResponse({
        type: "blocks",
        reverse: true,
        peer: { id: "uuid-blocker", identifier: "DEV-100", title: "Blocking issue" },
      }),
    );
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-100", title: "Blocking issue", type: "blocks", reverse: true },
    ]);
    // Reversed: source becomes target
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-blocker", relatedIssueId: "uuid-source", type: "blocks" },
    });
  });
  ...
  it("creates a 'blocks' relation when prose says 'blocks' (forward)", async () => {
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-200", title: "Downstream", type: "blocks", reverse: false },
    ]);
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-source", relatedIssueId: "uuid-target", type: "blocks" },
    });
  });
  ...
  it("creates a 'duplicate' relation when prose says 'duplicates'", async () => {
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-50", title: "The original", type: "duplicate", reverse: false },
    ]);
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-source", relatedIssueId: "uuid-orig", type: "duplicate" },
    });
  });
  ...
  it("creates a reversed duplicate when prose says 'duplicated by'", async () => {
    ...
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-dup", relatedIssueId: "uuid-source", type: "duplicate" },
    });
  });
  ...
  it("skips a reference already linked as related (outgoing)", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationsResponse({
        outgoing: [{ identifier: "DEV-3592", uuid: "uuid-target", type: "related" }],
      }),
    );
    ...
    expect(result.linked).toEqual([]);
    expect(result.skipped).toEqual([
      { identifier: "DEV-3592", existingType: "related", inferredType: "related" },
    ]);
  ...
  it("does NOT upgrade an existing 'related' to 'blocks' even when prose says 'blocked by'", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationsResponse({
        outgoing: [{ identifier: "DEV-100", uuid: "uuid-blocker", type: "related" }],
      }),
    );
    ...
    expect(result.linked).toEqual([]);
    expect(result.skipped).toEqual([
      { identifier: "DEV-100", existingType: "related", inferredType: "blocks" },
    ]);
    expect(rawRequest).toHaveBeenCalledTimes(1);
  });
  ...
  it("skips a reference already linked as blockedBy (incoming, normalized)", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationsResponse({
        incoming: [{ identifier: "DEV-100", uuid: "uuid-blocker", type: "blocks" }],
      }),
    );
    ...
    expect(result.linked).toEqual([]);
    expect(result.skipped).toEqual([
      { identifier: "DEV-100", existingType: "blockedBy", inferredType: "related" },
    ]);
  });
  ...
  it("scans comments when --include-comments equivalent input is provided", async () => {
    ...
    rawRequest.mockResolvedValueOnce(
      makeRelationCreateResponse({
        type: "related",
        reverse: false,
        peer: { id: "uuid-from-comment", identifier: "DEV-77", title: "From comment" },
      }),
    );
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-77", title: "From comment", type: "related", reverse: false },
    ]);
  });
  ...
  it("merges description + comments and prefers description's stronger inference", async () => {
    ...
    expect(result.linked).toEqual([
      { identifier: "DEV-100", title: "Blocker", type: "blocks", reverse: true },
    ]);
    expect(rawRequest).toHaveBeenNthCalledWith(2, ISSUE_RELATION_CREATE_MUTATION, {
      input: { issueId: "uuid-blocker", relatedIssueId: "uuid-source", type: "blocks" },
    });
  ...
  it("records a failed entry when resolveIssueId throws", async () => {
    const { rawRequest, resolveIssueId, graphQLService, linearService } = makeServices();
    rawRequest.mockResolvedValueOnce(makeRelationsResponse({}));
    resolveIssueId.mockRejectedValueOnce(new Error('Issue "GHOST-999" not found'));
  ...
  it("dryRun: reports what would be linked without calling create mutation", async () => {
    ...
    expect(rawRequest).toHaveBeenCalledTimes(1);
    expect(rawRequest).toHaveBeenCalledWith(GET_ISSUE_RELATIONS_QUERY, { id: "uuid-source" });
  });
  ...
  it("records failure when create mutation throws", async () => {
    ...
    expect(result.linked).toEqual([]);
    expect(result.failed).toEqual([{ identifier: "DEV-3592", reason: "rate limited" }]);
  });
});
