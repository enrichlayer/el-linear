...
function makeLinearService(impl: (id: string) => Promise<string>): LinearService {
...
describe("validateReferences", () => {
  ...
  it("deduplicates input identifiers", async () => {
    ...
    const result = await validateReferences(["DEV-100", "DEV-100", "DEV-200"], ls);
  ...
  it("does not throw when all resolutions fail", async () => {
    const ls = makeLinearService((id) => Promise.reject(new Error(`Issue "${id}" not found`)));
