...
export function formatTable(issues: LinearIssue[], fieldNames?: string[]): string {
...
export function formatCsv(issues: LinearIssue[], fieldNames?: string[]): string {
  const columnKeys = fieldNames ?? DEFAULT_COLUMNS;
  const columns = columnKeys
    .map((key) => ALL_COLUMNS[key])
    .filter((col): col is ColumnDef => col !== undefined);
...
export function formatMarkdown(issues: LinearIssue[], fieldNames?: string[]): string {
  const columnKeys = fieldNames ?? DEFAULT_COLUMNS;
  const columns = columnKeys
