...
export class LinearService {
  ...
  async getProjects(limit = 100): Promise<LinearProject[]> {
    ...
    const projectsWithData = await Promise.all(
      projects.nodes.map(async (project) => {
        const [teams, lead] = await Promise.all([project.teams(), project.lead]);
    ...
    if (statuses.nodes.length === 0) {
      ...
      if (statusName.toLowerCase() === "triage" && teamId) {
        const fallback = await this.client.workflowStates({
          filter: { name: { eqIgnoreCase: "Backlog" }, team: teamIdFilter(teamId) },
          first: 1,
        });
      ...
      throw notFoundError("Status", statusName, teamId ? `for team ${teamId}` : undefined);
    }
  ...
  async getLabels(teamFilter?: string, limit = 100): Promise<{ labels: LinearLabel[] }> {
    ...
    if (teamFilter) {
      ...
      for (const label of allLabels.nodes) {
        ...
        const teamRef = team ? { id: team.id, key: team.key, name: team.name } : undefined;
  ...
  async getCycleById(cycleId: string, issuesLimit = 50): Promise<LinearCycleDetail> {
