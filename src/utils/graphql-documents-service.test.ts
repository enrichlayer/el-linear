...
const { createGraphQLDocumentsService } = await import("./graphql-documents-service.js");
...
describe("GraphQLDocumentsService", () => {
    ...
    it("transforms document with all optional fields", async () => {
      ...
      mockRawRequest.mockResolvedValue({
        document: {
          ...
          issue: { id: "issue-1", identifier: "DEV-100", title: "Related Issue" },
  ...
  describe("createDocument", () => {
    ...
    it("throws on create failure", async () => {
      ...
      await expect(service.createDocument({ title: "Fail Doc" })).rejects.toThrow(
        'Failed to create document "Fail Doc"',
      );
    });
  });
  ...
  describe("updateDocument", () => {
    it("updates a document and returns transformed result", async () => {
      ...
      const result = await service.updateDocument("doc-1", { title: "Updated Title" });
      expect(result.id).toBe("doc-1");
    ...
    it("throws on update failure", async () => {
      ...
      await expect(service.updateDocument("doc-1", { title: "X" })).rejects.toThrow(
        "Failed to update document: doc-1",
      );
    });
  });
  ...
  describe("listDocumentsBySlugIds", () => {
    ...
    it("builds OR filter for multiple slugIds", async () => {
      ...
      const results = await service.listDocumentsBySlugIds(["slug-a", "slug-b"]);
      expect(results).toHaveLength(2);
      expect(mockRawRequest).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({
