...
describe("GraphQLService", () => {
  it("returns response data from rawRequest", async () => {
    mockRawRequest.mockResolvedValue({ data: { issue: { id: "123", title: "Test" } } });
  ...
  it("throws a readable error from GraphQL errors", async () => {
    ...
    await expect(service.rawRequest("{ foo }")).rejects.toThrow("Field 'foo' not found");
  });
  ...
  it("falls back to generic message when GraphQL error has no message", async () => {
    ...
    await expect(service.rawRequest("{ foo }")).rejects.toThrow("GraphQL query failed");
  });
